...
- **ベスト2ボール**（チーム内**参加メンバーの**ネット上位2名の合計・小さいほど良い。★2026-09-12 §3.5 追補で「メンバー」の定義を明確化）。

### 3.5 チーム対抗
- **★2026-09-12 追補【確定・load-bearing】（`docs/handoff/2026-09-12-nonparticipant-ghost.md` が正）**: 本節および §3.4 ベスト2ボールでいう「メンバー」は、一律に **`teamMembers(g,T)` ＝ `T.memberIds ∩ g.participants` かつ `state.players` に実在するもの** を指す（グロス対抗／ネット対抗／ホールバイホール／ベスト2ボール／ニアドラ本数／チーム勝ち点の配分のすべて）。`memberIds` に残っているが `participants` に居ない「ゴースト」は**一切集計しない**。`participants` に1人も居ないチームは**対抗の対象チームにならない**（`vegasStandings`／`m1Teams` の既存規則と同型）。`participants ⊇ memberIds` が成り立つ既存データでは**数値は1ビットも変わらない**（回帰スナップショット9ケース完全一致で実証）。
- **グロス対抗** = Σ effGross（メンバー）。小さいほど良い。
- **ネット対抗** = Σ net（メンバー）。小さいほど良い。
- **★2026-09-19 追補【確定・load-bearing・過去コンペの結果が変わる】（`docs/handoff/2026-09-19-team-avg-only.md` が正）**: グロス対抗／ネット対抗は **1人あたり平均**で判定する（**合計モードは廃止**・ユーザー確定）。式は常に **Σ（母数）÷ 母数**、**母数＝`teamScoreMembers(g,T)` ＝ `teamMembers(g,T)` ∩ 1H以上入力済み**（#186）、丸めは小数第1位、判定値＝表示値、同点は `add()` の山分け。#185 で一度導入した `g.teamScoreMode`（`'sum'|'avg'`）は**廃止**（既存データのフィールドは残置・非参照。`newGame` は書かず `migrate` も補完しない）。**これにより既存コンペを開いたときのグロス対抗／ネット対抗の値・勝者・勝ち点・配分額も平均で言い直される**（回帰25ケースのうち10ケースの値が変わり、うち3ケース＝人数が不揃いなケースで勝者・勝ち点・配分が変わることを実測。`nextKanji` は全ケース不変）。**ベスト2ボール・ホールバイホール・ラスベガス・1on1・ニアドラ・任意対決・大学対抗は非接触**（`renderTeams` の HTML バイト一致で担保）。人数不揃い警告（#195）は根拠が消えたため**撤去**。`g.teamScoreMode` は廃止フィールドとして扱う（旧・種目別チーム配点や `niapinHoles` と同じ後方互換パターン。回帰ケース `teamLegacySum` が「`'sum'` が残っていても平均で計算される」ことを固定）。
...
- **★2026-09-12 追補（手動上書き・設計確定／実装は該当PRで反映）**: 対象ホールは **par 自動導出（Par 3=ニアピン / Par 5=ドラコン）＋ホール単位の手動上書き**で決まる。上書きは `prizes.npdcOverride`（`{ "<holeIndex>": "np" | "dc" | "none" }`・**キーが無いホール＝自動**・既定 `{}` ＝従来と完全に同一）。Par 4 をドラコンにする／Par 5 をドラコンから外す等が可能で、**上書きはホール番号に固定され par 変更に追従しない**（自動に戻すのは幹事の明示操作のみ）。1ホールに1値なので **NP と DC は同一ホールに同居しない**（排他は構造的に保証）。廃止フィールド `niapinHoles`/`draconHoles` は**引き続き非参照**（再利用しない）。詳細・前後比較・受け入れ条件は `docs/handoff/2026-09-12-npdc-manual-holes.md` が正。
...
```jsonc
{
  ...
  "games": [ {
    ...
    "teams": [ { "id":"t1", "name":"チームレッド", "memberIds":["xxx"] } ],   // ★2026-09-12: memberIds は participants の部分集合であることを保証しない（参加者から一時的に外しても所属は残る＝可逆運用）。集計・表示側は必ず teamMembers(g,T) で絞る（§3.5 追補）
    "participants": ["xxx"],
    "scores": { "xxx": [4,5,null,...] },   // length 18, null=未入力
    "prizes": {
      "niapinHoles":[2], "draconHoles":[4],   // ★2026-08-20 廃止フィールド（非参照・後方互換のため残置。削除しない。2026-09-12 の手動上書きでも再利用しない）
      "npdcOverride": { "3":"dc", "2":"none" },   // ★2026-09-12 対象ホールの手動上書き。キー=ホールindex / 値="np"|"dc"|"none"（キー無し=自動＝par導出）。既定 {} ＝従来と完全に同一（§3.6 追補）
...
rooms/{roomCode}                      // 1コンペ=1ルーム
  ...
  prizes/meta:        { niapinHoles[], draconHoles[], npdcOverride{}, niapinWinner{}, draconWinner{}, twoSets, niapinWinner2{}, draconWinner2{} }
```
...
- **★2026-09-20 追補（`docs/handoff/2026-09-20-roulette-undo.md` が正）**: ハーフ境界の再付与（`R.cur===9` での `remChange`/`remChallenge` 上書き）は**揮発スナップショット1件**（`rl.undo`・メモリのみ・`golfCompe_v1` に保存しない）で取り消せる。なお上書き直前の `remChange` は `rlCanAdvance`（`left=8-cur`）により**必ず全チーム0**であることが保証されるため、非可逆なのは **`remChallenge` の前半余りだけ**（取り消しではこれも復元する）。取り消しは進行中のコンペ（`gid` 一致・同一セッション）に限られ、**過去コンペの結果は書き換えられない**（スナップショットを保存しないので構造的に不可能）。本節のルール（前半の余りは失効・使い切り必須）**そのものは不変**。あわせて STOP はスピン開始から 800ms 受け付けない（START と STOP が同座標に描かれるための誤爆防止・`game.roulette` のフィールドは増やさない）。
...
- **チーム戦スコア表**：右上に **グロス/ネット/ホールバイホール スイッチ（3択・既定ネット）**。**チーム＝チーム成績順**（グロス／ネット＝**1人あたり平均の昇順**〔★2026-09-19・#198 で「Σ合計の昇順」から変更。`teamGrossVal`/`teamNetVal` を呼ぶ＝順位カードと同じ正〕／HBH=取得ホール数**降順**）。**チーム内メンバー＝個人スコア順**（グロス=effGross／ネット=netScore／**HBH時はネット順**）。状態 `scSortTeam`。
...
**§3 追補（新規純関数1つのみ・既存計算は不変）**: 幹事がチームごとに入力したポイント（`g.custom.pts[teamId]`）だけで勝敗を決めるチーム種目。スコア・ハンデからは一切計算しない。`customPts(g,T)` は未入力（キー無し／空文字／非数）を `null`（対象外）として返し、`0` は正規の値として扱う。`teamWinPoints` に `add('customMatch', teams.map(customPts), 'desc')` を**末尾に1件追加**（最大が勝ち・同点は既存どおり `w/n` 山分け・入力が1チーム以下なら `live<2` で種目不成立）。既存種目の判定値・`events` の既存要素・`wins` の index 対応は不変。あわせて `anyTeamEvent`（ニアドラのチーム種目成立条件）に `F.customMatch` を含める。
**§4 追補（後方互換・migrate 補完）**: `formats.customMatch`: bool（既定 false・α＝`BETA_FMT` には入れない）／`game.custom`: `{ name:'', pts:{} }`（`name`=種目名の自由入力・20文字上限・空なら i18n `term.custom` にフォールバック／`pts`= **teamId → number** のマップ。チーム削除時の孤児キーは掃除せず非参照で放置。チーム並べ替えはIDキーのため無影響）／`points.teamEventPts.customMatch`: 1（per-key バックフィルで自動補完）／`announced.customMatch`（object キー追加のみ）。localStorage キー・既存フィールドは不変。表示は結果発表＞チーム戦の**最後**のタブ（大型ヒーロー＋順位バッジ＋勝ち/引分の文字併記）、入力UIはそのタブ内の折りたたみ（§11.14「幹事の操作UIは控えめ配置」）。
