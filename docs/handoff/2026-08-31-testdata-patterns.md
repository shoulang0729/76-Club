...
→ ホール6〜17 は A/B が完全同一（コピー）＝すべてハーフ。**チームサマリ「レッド 3 – 1 ブルー・AS 1」**、個人配点は **A側 +2/+0/+1/+2/+2**、B側 +0/+2/+1/+0/+0（★2026-08-31 訂正: 初版本文の「A側 +2/+2/+1/+2/+2」は同節の表〔#2 は B の 3UP 勝ち〕および `computePoints` の実装と矛盾する誤記。実装は表どおり）。`teamWinPoints` の `match1v1` はマッチ勝利数 3 vs 1 でレッド勝ち。
