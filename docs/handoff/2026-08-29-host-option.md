...
1. **対象順位の選択**: 1位／2位／ブービーを**個別 ON/OFF（複数選択可）**。既存の「2位＋ブービー」は `{r1:OFF, r2:ON, booby:ON}` で表現。
2. **マスタースイッチ**: 既存 `kanjiBadge` を**維持**する【既定で確定】。
   - 根拠: PR #85 で出荷済みの UI 連続性・1タップで機能全体を殺せる・OFF にしても順位選択の設定値が保存されたまま残る。
   - `kanjiBadge=ON` かつ 3順位すべて OFF → **バッジなしの有効な状態**（エラー・強制補正はしない。注記文で示すのみ）。
3. **繰り上げ／繰り下げ**: ゲームごとの単一設定 `kanjiShift`（'down'｜'up'・全対象順位に共通適用）。順位ごとの方向設定は**しない**（UI 過剰・§16 Q2 に代替案として記載）。

## §11 意味論の厳密定義（判定式・load-bearing）

### §11.1 順位リスト
`ranked(parts, netScore, 'asc')` の並び（現行 nextKanji と同じ: 参加者のうち `gross>0`・タイブレーク §10.19 込み）。リスト長 = N。**位置**: 1位 = idx0・2位 = idx1・ブービー = idx N-2。位置が存在しない（idx<0 または idx≥N）対象は**該当なし**。

### §11.2 シフト（免除時の代替探索）
対象位置の選手が `kanjiExempt` の間、**位置を1つずつ移動**して最初の非免除者を採用する:
- **`'down'`＝繰り下げ（既定）**: 順位数値が**大きい**方向（1位→2位→…→最下位）。
- **`'up'`＝繰り上げ**: 順位数値が**小さい**方向（…→2位→1位）。
- ブービーにも同じ絶対方向を適用: 繰り下げ＝**ブービーメーカー（最下位）方向**・繰り上げ＝上位方向。
- 連続免除はそのまま歩き続ける（§13 例G）。
- **端の打ち切り**: リスト端を越えたら**その対象は該当なし**（逆方向へのフォールバックはしない）【既定で確定・単純で予測可能。免除が端まで連続するのはレアケース】。

### §11.3 重複の扱い
複数の対象順位が**同一人物**に解決した場合（例: N=3 で2位とブービーが同位置・シフトの合流）、**バッジは1個**（再シフトして別人に散らさない）【既定で確定・現行実装も hl マップ上書きで1個＝同等】。

### §11.4 実装スケッチ（js/calc.js `nextKanji` 置き換え）
返り値を `{nikai,booby,sameCount}` から **pid 配列（重複除去済み・該当なしは空配列）**へ変更。呼び出し元は results.js の net 分岐のみ（§1 #2）。

```js
/* next kanji（§11.17）: 対象順位 kanjiRanks を kanjiShift 方向に免除スキップして解決。表示専用・配点非関与 */
function nextKanji(g){
  const parts=g.participants.filter(pid=>{const p=state.players.find(x=>x.id===pid); return p && gross(g,pid)>0;});
  const order=ranked(parts,pid=>netScore(g,pid),'asc').map(r=>r.pid);
  const N=order.length, R=g.kanjiRanks, step=g.kanjiShift==='up'?-1:1;
  const exempt=pid=>{const p=state.players.find(x=>x.id===pid); return !p||p.kanjiExempt;};
  const resolve=i0=>{ for(let i=i0; i>=0&&i<N; i+=step){ if(!exempt(order[i])) return order[i]; } return null; };
  const targets=[]; if(R.r1)targets.push(0); if(R.r2)targets.push(1); if(R.booby)targets.push(N-2);
  return [...new Set(targets.filter(i=>i>=0&&i<N).map(resolve).filter(Boolean))];
}
```
...
### §11.5 旧 filter 方式との違い（明記）
旧実装は「免除者を除外したリストで 2位/ブービーを数え直す」ため、**免除者の位置によって無関係な対象まで動く**副作用があった（例D: 最下位が免除だとブービー本人が非免除でもブービーが繰り上がる）。新方式は**対象位置起点のシフト**なので、免除者が対象位置とシフト経路上にいない限り結果は動かない。この差は §13 の前後比較で網羅。

## §12 データモデル（§4 追補・正本 §11.17）

**追加フィールド（いずれも `golfCompe_v1` 内・localStorage 新キーなし）**:

| フィールド | 型 | 既定 | 意味 |
|-----------|----|------|------|
| `game.kanjiRanks` | `{r1:boolean, r2:boolean, booby:boolean}` | `{r1:false, r2:true, booby:true}` | 対象順位（既定＝現行 ON 挙動と同じ2位＋ブービー） |
| `game.kanjiShift` | `'down'` \| `'up'` | `'down'` | 免除時のシフト方向（§11.2） |

- `kanjiBadge:boolean`（既定 false）は**そのまま維持**（マスタースイッチ・§10-2）。
- `js/state.js` migrate 追記: `if(!g.kanjiRanks) g.kanjiRanks={r1:false,r2:true,booby:true}; if(!g.kanjiShift) g.kanjiShift='down';`（kanjiBadge の値に関わらず補完。**旧 kanjiBadge=true のゲームは自動的に「2位＋ブービー・繰り下げ」となり §13 例A/B の範囲で現行と同一表示**）。`newGame()` にも同値を追加。
- バックアップは state 丸ごとのため自動追従（backup.js 変更なし）。旧バックアップ取込は migrate 補完。

## §13 前後比較例（判定式変更・必須）

前提: N=10・ネット順 P1(1位)〜P10(最下位)・P9=ブービー。「旧」= PR #85 時点（filter 方式・2位＋ブービー固定）。「新既定」= kanjiBadge:ON・r2+booby・down。

| 例 | 免除者 | 旧 | 新既定(down) | 新 up | 差の説明 |
|----|--------|----|--------------|-------|---------|
| A | なし | P2, P9 | P2, P9 | P2, P9 | 一致（最頻ケース） |
| B | P2 | P3, P9 | **P3**, P9 | **P1**, P9 | down は旧と一致。up は2位→1位へ繰り上げ |
| C | P9(ブービー) | P2, **P8**（上位方向相当） | P2, **P10**（BM方向） | P2, **P8** | **既定 down は旧と差**（§16 Q2）。up なら旧一致 |
| D | P10(BM) | P2, **P8**（ブービーが動く副作用） | P2, **P9** | P2, **P9** | 新方式は非免除のブービー本人のまま＝直感的（§11.5） |
| E | P1（r1:ON・up の場合） | —（旧に1位対象なし） | （down なら P1→P2…） | 1位**該当なし**（端打ち切り §11.2） | r2/booby は独立に判定継続 |
| F | なし（N=3・r2+booby） | P2 に1個（eligible[1]=eligible[len-2]） | P2 に**1個**（idx1 が両対象・§11.3） | 同左 | 重複はバッジ1個・旧同等 |
| G | P2,P3 連続免除 | P4, P9 | **P4**, P9（2つ歩く） | **P1**, P9 | 連続免除は歩き続ける |

- 数値（ネット・pt・¥）は全ケースで不変＝**変わるのはバッジの付く行のみ**。
- 既定 OFF（kanjiBadge=false）のゲームは新旧とも表示ゼロで差なし。

## §14 設定UI（js/game.js「次回幹事」カードの拡張）

マスター ON のときだけサブ設定を表示（Vegas 個別設定カードの出没と同型）。`setKanjiBadge` に `renderGame()` を追加して出没を再描画。

```
┌ 次回幹事 ─────────────────────────┐
│ ☑ 次回幹事のバッジを表示                    ← 既存（文言から「（ネット2位とブービー）」を削除）
│ ─ 以下はマスターON時のみ表示 ─
│ 対象順位:  ☐ 1位   ☑ 2位   ☑ ブービー      ← チェック3つ横並び（fmtgrid 風 inline label）
│ 幹事対象外の選手だった場合: [下の順位へ（例: 2位→3位）▼]   ← select（vegas cap と同型）
│                              ├ 下の順位へ（例: 2位→3位）＝既定
│                              └ 上の順位へ（例: 2位→1位）
│ （muted）ネット順位のうち選んだ順位に「幹事」バッジを表示。
│  対象の選手が「幹事対象外」のときは選んだ方向の次の選手へ。
└──────────────────────────────┘
```
...
```js
function setKanjiRank(k,v){ curGame().kanjiRanks[k]=v; save(); }
function setKanjiShift(v){ curGame().kanjiShift = v==='up'?'up':'down'; save(); }
function setKanjiBadge(v){ curGame().kanjiBadge=v; save(); renderGame(); }   // 既存＋再描画追加
```

- ラベルの「下の順位へ／上の順位へ」に**具体例（2位→3位）を必ず併記**する（繰り上げ/繰り下げの語義の取り違え防止・§16 Q1）。

## §15 i18n（追加6キー＋文言更新2キー・ja/zh/en 同時）
...
| `game.kanjiR2` | 2位 | 第2名 | 2nd |
| `game.kanjiBooby` | ブービー | 倒数第2名 | Booby |
| `game.kanjiShiftL` | 幹事対象外の選手だった場合 | 若该选手为干事豁免 | If that player is organizer-exempt |
| `game.kanjiShiftDown` | 下の順位へ（例: 2位→3位） | 顺延到下一名（例: 第2名→第3名） | Next rank down (e.g. 2nd → 3rd) |
| `game.kanjiShiftUp` | 上の順位へ（例: 2位→1位） | 提前到上一名（例: 第2名→第1名） | Next rank up (e.g. 2nd → 1st) |

（7キー。表題の「6」は誤りでこの表が正）

**文言更新（キーは不変・値のみ）**: `game.kanjiTgl`＝「次回幹事のバッジを表示」（順位の固定記載を削除。zh/en も同旨）／`game.kanjiNote`＝「ネット順位のうち選んだ順位（1位・2位・ブービー）に「幹事」バッジを表示します。対象の選手が幹事対象外のときは選んだ方向の次の選手に移ります。」（zh/en 同旨・実装時に3言語同時更新）。

## §16 ユーザー確認事項（一括質問・回答待ち）

- **Q1 語義の確認**: 「繰り下げ」＝**対象位置が下位方向へ動く**（2位免除→3位の人が幹事）、「繰り上げ」＝上位方向（2位免除→1位の人）と定義した。逆の読み（「3位の人が2位に繰り上がって幹事」＝結果は同じく3位の人）と紛らわしいため、**UI では方向語を使わず「下の順位へ（例: 2位→3位）」の具体例表記**にしている。この定義でよいか。
- **Q2 既定方向とブービーの扱い**: 既定 `'down'`（繰り下げ）を提案。ただし**ブービー免除時は現行実装（上位方向相当）と挙動が変わる**（§13 例C: 旧 P8 → 新 P10=ブービーメーカー）。現行は 2位=下方向・ブービー=上方向の混合で、単一方向設定では完全互換にできない。選択肢: (a) 提案どおり統一 down（BM 幹事も慣例として存在・シンプル）／(b) 既定 up（ブービー側は現行維持・2位側が変わる）／(c) 順位ごとに方向を持つ（UI・データ複雑化のため非推奨）。**(a) で進めてよいか**。

（マスタースイッチ維持・重複時バッジ1個・端打ち切り＝該当なし、は既定で確定・§10〜§11 に根拠記載。異論があれば併せて）

## §17 受け入れ条件・触らない範囲・PR分割（追補分）

**受け入れ条件**（§7 に追加。前提: テストデータ・kanjiExempt を付け替えながら確認）:
1. 旧データ互換: PR#85 時点で kanjiBadge=ON だったゲームは migrate 後「2位＋ブービー・下の順位へ」となり、免除者なしなら表示が完全一致（§13 例A）。
2. 対象順位: 1位/2位/ブービーの各チェックが独立に効く（1位のみON→ネット1位の行だけにバッジ等）。全OFF→バッジなし（エラーなし）。
3. シフト: 2位を免除にして「下の順位へ」→3位に、「上の順位へ」→1位にバッジ。ブービー免除で down→最下位・up→下から3番目（§13 例C）。連続免除で2つ歩く（例G）。
4. 端打ち切り: 1位ON・up・1位免除 → 1位対象は該当なし・他対象は正常（例E）。
5. 重複: N=3・r2+booby で同一人物にバッジ1個（例F）。
6. サブ設定はマスターON時のみ表示され、OFF→ONで設定値が保持されている。
7. ネット順位・数値・配点・他タブは全設定で不変。`node tools/verify.mjs` green・3言語切替OK・`?v=`更新。

**触らない範囲**（§8 に追加): `kanjiBadge` の既定 false・`ranked`/タイブレーク・`term.organizer`・rankCardNS の hlMap 機構・kanjiExempt の選手タブUI。

**推奨PR分割**: **1 PR で完結**。js/calc.js（nextKanji 置き換え §11.4）・js/state.js（migrate/newGame 2行）・js/game.js（カード拡張＋セッター §14）・js/results.js（net 分岐の hl 組み立て §11.4）・js/i18n.js（追加7キー＋更新2キー×3言語）・index.html（`?v=` のみ）。
