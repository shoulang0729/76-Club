...
**まとめ出しとモック承認**（2026-08-20 制定）:
- 大きな構造変更（ナビ・画面構成・表示レイアウトの再編）は**テーマ単位で要件をまとめて**受け、architect 設計の冒頭で不明点を一括質問する（五月雨の追加要件は設計追補コストが大きい）。
- **UI テーマは実装着手前にモック（スクリーンショット等）をユーザーに提示して承認を得る**（`/feature` 手順 1.5）。見た目の方向違いをモック1枚（≈10分）で検出し、実装1周（≈30分）の手戻りを防ぐ。小さな磨き（文言・余白・色）は S レーンで随時・モック不要。
