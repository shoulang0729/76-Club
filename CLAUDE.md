...
### ブラウザ検証の選び方（2026-09-19 制定・★網羅しない）
`verify.mjs` / `regress.mjs` は**必ず全実行**。そのうえでブラウザ実測は、**「その変更が壊しうるもの」から逆算して最小限を選ぶ**。幅×言語×テーマの全組合せ掃引は**しない**（遅いうえに、何を守っているか説明できない検証は弱い）。

| 変更の種類 | 壊れ方 | 測るもの |
|---|---|---|
| **§3 の計算** | 数値が変わる | **今までどおり全数**（オプション全組合せ・回帰全ケース・前後比較）。ここは削らない |
| 文字サイズ・寸法の式 | 溢れる／折り返す | **最悪ケース1点**＝「一番狭い幅 × 一番長い文字列」。**テーマは測らない**（レイアウトに影響しない） |
| 色・トークン | 片方のテーマで読めない | 両テーマ × 1幅 × 1言語 |
| 列数・グリッド・`@media` | 境界で崩れる／跳ぶ | **境界の前後2点ずつ**。刻み掃引はしない |
| 複数箇所で共用する関数・部品（`card()` / `renderScorecard` 等） | 巻き添え | **他経路の HTML バイト比較を1発**（スクリーンショット多数より速く確実） |
| 純粋な分岐ロジック（列数計算など） | 条件の取り違え | **ブラウザ不要**。vm / node で直接検算する |

- 言語は「**文字列が一番長いもの**」を選ぶ（区分ラベル等は en が最長・チーム名等は ja/zh が最長）。3言語とも回すのは i18n パリティ（`verify.mjs` が担保）とは別問題なので、**長さが効く箇所だけ**。
- スクリーンショットは**レビューで人が見る1枚**に絞る。差分の証明には使わない（証明は機械比較で行う）。
- **PM の再検証は機械検証できないものだけ**（設計との突き合わせ・文言・見た目）。`verify` / `regress` の二度打ちはしない。

### サイズ判定の運用（S レーンを実際に使う）
`§3 計算・データモデル・localStorage・i18nキー集合・タブ/モジュール構成`に**非接触**なら S。**S は architect を省略**し、PM が受け入れ条件つきの簡潔な Issue を書いて implementer に直接渡す（設計書を書かない）。1ファイル数行の変更に 600 行の設計書を書かない。
